...
[role="_abstract"]
Configure intermediate backup and restore persistent volume claims (PVCs) for Data Mover operations to optimize data movement and improve backup performance for storage classes like CephFS. This provides faster backups by using read-only volumes from snapshots.
