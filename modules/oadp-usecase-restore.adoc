...
To restore a backup of an application by using {oadp-short}, you create a restore Custom Resource (CR) as shown in the following example. In the example, you restore the backup to a new target namespace, `test-restore-application`. The source namespace refers to the application namespace that you included in the backup. You then verify the restore by changing your project to the new restored namespace and verifying the resources.
...
[source,terminal]
----
$ oc apply -f <restore_cr_filename> # <1>
----
<1> Specify the file name of the restore CR manifest.
...
[source,terminal]
----
$ oc describe restore <restore_name> -n openshift-adp # <1>
----
<1> The name of the restore.
