...
[role="_abstract"]
Access restored virtual machine files through SSH by using `scp` or `sftp` with the `VirtualMachineFileRestore` (VMFR) CR. This helps you to transfer files from VM backups efficiently.

When you configure SSH access, the VMFR controller automatically generates an SSH key pair and stores it in a Kubernetes secret. The default SSH username is `oadp`. The SSH file server listens on port `2222`.
