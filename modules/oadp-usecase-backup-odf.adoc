...
[id="oadp-usecase-backup_{context}"]
= Backing up an application using {oadp-short} and {odf-short}

In this use case, you back up an application by using {oadp-short} and store the backup in a storage object provided by {odf-first}.
...
[source,terminal]
----
$ oc extract --to=- secret/test-obc # <1>
----
<1> `test-obc` is the name of the OBC.
+
.Example output
...
----
<1> Specify the AWS_ACCESS_KEY_ID.
<2> Specify the AWS_SECRET_ACCESS_KEY.
...
. Configure the DPA as shown in the following example:
+
.Example DPA
[source,yaml]
----
...
<2> The S3 URL of {odf-short} storage.
...
[source,terminal]
----
$ oc apply -f <dpa_filename> # <1>
----
<1> Specify the DPA file name.

. Verify that the DPA is created successfully by running the following command. In the example output, you can see the `status` object has `type` set to `Reconciled`. This means, the DPA is successfully created.
+
[source,terminal]
----
$ oc get dpa -o yaml
...
[source,terminal]
----
$ oc apply -f <backup_cr_filename> # <1>
----
<1> Specify the backup CR file name.
...
[source,terminal]
----
$ oc describe backup test-backup -n openshift-adp # <1>
----
