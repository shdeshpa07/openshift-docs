...
`snapshot.storage.kubernetes.io/is-default-class`:: Specifies the `VolumeSnapshotClass` configuration file to use when none is specified by `VolumeSnapshot`. Where `VolumeSnapshot` is a request for snapshot of a volume.
