...
[id="oadp-about-updating-default-bsl_{context}"]
= About updating default backup storage location in a DPA

[role="_abstract"]
Review why updating the `default` field of the `backupLocations` object in a `DataProtectionApplication` (DPA) does not change the default `BackupStorageLocation` (BSL) when multiple backup locations are configured. This helps you to understand the root cause and apply the correct workaround.
...
Workaround::
To change the default BSL, delete and re-create the DPA. Deleting and re-creating the DPA ensures that the BSL resources are created from scratch. The new `default` setting is applied during initial creation.
+
--
...
where:
+
`<dpa_name>`:: Specifies the name of the DPA custom resource.
...
where:
+
`<updated_dpa_file>`:: Specifies the file name of the updated DPA custom resource definition.
--
