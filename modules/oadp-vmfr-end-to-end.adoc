...
----
+
Replace `<vm_key_name>` with a name for the SSH key file.
...
`<path_to_public_key>`:: Specifies the path to the SSH public key file.
...
`<vm_namespace>`:: Specifies the namespace for the VM.
`<ssh_secret_name>`:: Specifies the name of the secret that contains the SSH public key.
`<vm_user>`:: Specifies the username for the VM.
...
. Apply the `VirtualMachine` CR by running the following command:
+
[source,terminal]
----
$ oc apply -f <vm_cr_filename>
----
...
$ virtctl ssh <vm_user>@<vm_name> \
  ...
  -c "echo 'Test file for VMFR validation - $(date)' > /home/<vm_user>/test-vmfr-file.txt"
----
...
----
$ virtctl scp id-rsa \
  <vm_user>@<vm_name>:/home/<vm_user>/id-rsa \
  -n <vm_namespace> \
  -i ssh/<vm_key_name>
----
