...
[role="_abstract"]
By configuring multiple network interface controllers (NICs), you can provide dedicated network links in the node virtual machines (VMs) for uses such as storage or databases. {product-title} clusters on {vmw-first} support connecting up to 10 network NICs to a node.
