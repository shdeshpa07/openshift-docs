...
[source,terminal,subs="attributes+"]
----
$ oc adm must-gather --image={must-gather-v1-3} -- /usr/bin/gather --request-timeout <timeout>
----
+
Replace <timeout> with a timeout value.
+
** For {oadp-short} 1.4, run the following command:
+
[source,terminal,subs="attributes+"]
...
[source,terminal,subs="attributes+"]
----
$ oc adm must-gather --image={must-gather-v1-3} -- /usr/bin/gather --request-timeout 15s --skip-tls=true
----
+
