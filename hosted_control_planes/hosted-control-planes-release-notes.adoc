...
In {hcp} on {ibm-power-name}, you can create heterogeneous node pools on agent hosted clusters. Heterogeneous node pools support different configurations so that you can create pools that are optimized for various workloads. For more information, see xref:../hosted_control_planes/hcp-deploy/hcp-deploy-ibm-power.adoc#hcp-ibm-power-create-heterogeneous-nodepools-agent-hc-con_hcp-deploy-ibm-power[Creating heterogeneous node pools on agent hosted clusters].

[id="hcp-4-18-dual-stack_{context}"]
==== {hcp-capital} in a disconnected environment on a dual-stack network

In this release, {hcp} in a disconnected environment on a dual-stack network is a General Availability feature. For more information, see xref:../hosted_control_planes/hcp-disconnected/hcp-deploy-dc.adoc[Introduction to {hcp} in a disconnected environment].
